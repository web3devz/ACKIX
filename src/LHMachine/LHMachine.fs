...
// Compile the given Ackix Machine instruction into one or several
// TVM instructions.
let rec AckixInstructionToTVM (i:Instruction) : list<TVM.Instruction> =
    match i with
    | Null -> [TVM.PushNull]
    | False -> [TVM.False]
    | True -> [TVM.True]
    ...
    | Function b ->
        let body =
            b
            |> List.map AckixInstructionToTVM
            |> List.concat
        [TVM.PushCont body]
